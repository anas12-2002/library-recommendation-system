...
export async function getBooks(): Promise<Book[]> {
  try {
    ...
    console.log('API Response:', data); // للتحقق من هيكل البيانات
    
    // إذا كانت البيانات تحتوي على body (كـ string)
    if (data.body) {
      // parse body من string إلى array
      const booksArray = JSON.parse(data.body);
      console.log('Parsed books:', booksArray);
      return booksArray;
    }
    
    // إذا كانت البيانات مباشرة كـ array
